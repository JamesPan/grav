...
hljs: true
jscomments:
  id: /2016/01/15/do-not-forget-to-indicate-the-source/
  title: 如果你转载文章不注明出处
  url: http://blog.jamespan.me/2016/01/15/do-not-forget-to-indicate-the-source/
routes:
  aliases:
  - posts/do-not-forget-to-indicate-the-source
  default: /2016/01/15/do-not-forget-to-indicate-the-source
taxonomy:
  category:
  - blog
  - Study
...
![](https://i.imgur.com/ALauRuQl.png)
...
pattern 域是这个插件要识别的模式，默认配置中 pattern 域的值为 `indicate-the-source`，那么插件就会去寻找符合正则模式 `` 的字符串，全部替换为「转载请注明出处」信息。这里使用注释作为信息插入位置，巧妙地实现了功能点 2。
...
然后在文章的 Markdown 源码中找一处插入 ``，

![](https://i.imgur.com/C2EMBmFl.png)

就会得到这样的效果：

![](https://i.imgur.com/Pv2YQeal.png)
...
对于已经存在的文章，我可以在注释 `===` 的后面追加 ``，反正每篇文章都有这个 more 注释，反正「转载请注明出处」信息放在哪里并不重要，放了就差不多了。
